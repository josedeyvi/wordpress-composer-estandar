...
class FullCulqi_Ajax {
	...
	public function get_merchants() {
		...
		$url_get_merchants = esc_url_raw( $_GET['url_merchant'] );
	...
	public function get_merchant() {
		$token            = sanitize_text_field( $_GET['token'] );
		$url_get_merchant = esc_url_raw( $_GET['url_merchant'] );
	...
	public function sync_charges() {
		...
		? sanitize_text_field( $_POST['after_id'] )
		: '';
	...
	public function sync_orders() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error(
				esc_html( 'You do not have permission.', 'fullculqi_subs' )
			);
		}
